...
{
    [AddComponentMenu("")]
    public class NetworkManagerStackedPrediction : NetworkBehaviour
    {
        [Header("Spawns")]
            ...
            for (int spawnX = 0; spawnX < sqrt; ++spawnX)
            {
                for (int spawnY = 0; spawnY < sqrt; ++spawnY)
                {
                    ...
                    if (spawned < spawnAmount)
                    {
                       ...
                       NetworkObject go =   Sandbox.NetworkInstantiate(spawnPrefab, new Vector3(x, y, 0), Quaternion.identity);
                        ++spawned;
                    }
                }
            }
