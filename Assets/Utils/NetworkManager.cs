...
[DisallowMultipleComponent]
    ...
    public partial class NetworkManager : NetworkCherry
    {
        ...
        [Header("Spawn Points")]
        public Transform SpawnPosition; // 保留单个默认生成点
        public Transform[] SpawnPoints; // 新增的多个生成点数组
    
        [Header("Random Spawn")]
        public bool UseRandomSpawnPoints = true; // 是否启用随机生成点
    
        private HashSet<int> _usedSpawnIndices = new HashSet<int>();
        ...
        public void OnPlayerConnected(NetworkSandbox sandbox, NetworkPlayer client)
        {
            Transform spawnPoint = GetSpawnPoint();
            Vector3 spawnPos = spawnPoint.position;
        
            if (StaggerSpawns && !UseRandomSpawnPoints)
            {
                spawnPos += (HorizontalOffset * Vector3.left) * (sandbox.ConnectedPlayers.Count - 1);
            }
        
            var player = sandbox.NetworkInstantiate(PlayerPrefab, spawnPos, spawnPoint.rotation, client);
        ...
        private Transform GetSpawnPoint()
        {
            ...
            // 如果还没有配置任何生成点，使用默认生成点
            if (SpawnPoints == null || SpawnPoints.Length == 0)
            {
                Debug.LogWarning("No spawn points configured, using default spawn position");
                return SpawnPosition;
            }
        
            // 如果所有生成点都已使用过，重置使用记录
            if (_usedSpawnIndices.Count >= SpawnPoints.Length)
            {
            ...
            if (availableIndices.Count > 0)
            {
                ...
                _usedSpawnIndices.Add(randomIndex);
                return SpawnPoints[randomIndex];
            }
        
            // 如果找不到可用生成点（理论上不应该发生），返回默认生成点
            return SpawnPosition;
        }
        // This is called on the server when a player has disconnected.
        public  void OnPlayerDisconnected(NetworkSandbox sandbox, Netick.NetworkPlayer client, TransportDisconnectReason transportDisconnectReason)
        {
